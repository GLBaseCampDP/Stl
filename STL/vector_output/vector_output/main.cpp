...
int main() {    MESSAGE("Vector Output:");
   ...
   MESSAGE(" # for & value: "); 	// доступ за значенням
   for (char n : v) cout << n;

   MESSAGE(" # for & reference: "); 	// доступ за посиланням
   for (const char& n : v) cout << n;
   MESSAGE(" # for & : "); 	// доступ за const&
   for (auto&& n : v)    cout << n;

   MESSAGE(" # iterator: ");
   for (vector<char>::iterator it = v.begin(); it !=
      v.end(); ++it)     cout << *it;
